...
#' @import httr
#' @export
run_ashe_sample_summary <- function(data, name, selection_cuts) {
  ...
  # full time ratio
  fulltime_ratio <-  nrow(data[data$Ft==1,])/n_rows
  print(paste0("Ratio of full time workers: ", fulltime_ratio))

  # part time ratio
  parttime_ratio <-  nrow(data[data$Ft==2,])/n_rows
  ...
  permanent_ratio <-  nrow(data[data$Pt==1,])/n_rows
  print(paste0("Ratio of full time workers: ", permanent_ratio))

  # temp work ratio
  temp_ratio <-  nrow(data[data$Pt==2,])/n_rows
...
#  file_name <- paste0(file_name,name)
  ...
  cat(paste0('Cuts previously applied to the data: ',selection_cuts), file = log_con, append=TRUE, sep = "\n")
  ...
  cat(paste0(" from: ", min(data["year"])), file = log_con, append=TRUE)
  cat(paste0(" to: ", max(data["year"])), file = log_con, append=TRUE, sep = "\n")
